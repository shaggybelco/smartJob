...
import { PIPELINE_STATUSES, type AppStatus, type Application } from "@smartjob/shared";
...
export function BoardPage() {
  ...
  const grouped: Record<AppStatus, Application[]> = {
    APPLIED: [], SCREENING: [], INTERVIEW: [], OFFER: [], REJECTED: [], WITHDRAWN: [],
  };
  for (const a of items) grouped[a.status].push(a);
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Your applications grouped by stage. Open a card to update its status.
        </p>
      </div>
...
function Card({ application: a }: { application: Application }) {
  const linked = !!a.jobApplicationId;
  return (
    <Link
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        {linked && (
          <span
            className="pill bg-brand-100 text-brand-700 dark:bg-brand-900/40 dark:text-brand-200"
            title="In-platform application"
          >
            IN
          </span>
        )}
      ...
      {a.salary && (
        <div className="mt-1 text-[11px] tabular-nums text-slate-500">{formatZar(a.salary)}</div>
      )}
    </Link>
  );
}
